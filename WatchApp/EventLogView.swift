...
struct EventLogView: View {
    ...
    /// Present during a live code: enables the undo row, and reading the
    /// events THROUGH the engine keeps the list observing it, so an undo
    /// redraws immediately.
    var engine: SessionEngine? = nil

    private var liveEvents: [CodeEvent] { engine?.session.events ?? events }
    ...
    var body: some View {
        Group {
            if liveEvents.isEmpty {
                ...
                List {
                    if let engine { UndoLastRow(engine: engine) }
                    ForEach(liveEvents.reversed()) { event in
                        HStack(spacing: 6) {
